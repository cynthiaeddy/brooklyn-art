...
app.get('/searchbytag/:value', (req, res, next) => {
  ...
  axios
    .get(
      `https://api.collection.cooperhewitt.org/rest/?method=cooperhewitt.search.objects&access_token=${process.env.COOPER_API_TOKEN}&has_images=1&per_page=300&tag=${value}`
    )
    .then(response => {
      console.log(
        'response length for keyword:',
        req.params,
...
app
  .listen(port, () => {
    console.log(
      `Let's steal some art! Listening on port: ${port}, in ${app.get(
        'env'
      )} mode.`
    )
  })
